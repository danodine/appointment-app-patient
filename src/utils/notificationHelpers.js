...
export async function registerForPushNotificationsAsync(userId) {
  ...
  if (Platform.OS === "android" || Platform.OS === "ios") {
    ...
    try {
      console.log("🚀 About to call getExpoPushTokenAsync()...");
      const tokenResponse = await Notifications.getExpoPushTokenAsync();
      token = tokenResponse.data;
      console.log("✅ Token received:", token);
    } catch (err) {
      console.error("❌ Error while getting Expo Push Token:", err);
    }
    console.log("Requesting Expo Push Token... 22222");
    ...
    try {
      ...
      await axiosInstance.patch(
        `http://192.168.0.63:3000/api/v1/users/updateExpoPushToken`,
        { token }
      );
  ...
  const responseListener =
    Notifications.addNotificationResponseReceivedListener((response) => {
      ...
      if (data?.appointmentId) {
        console.log(
          "Navigate to appointment details for ID:",
          data.appointmentId
        );
